...
\begin{equation}
\overline{\text{STRAIN}}_{i} = \frac{1}{n} \sum_{t=1}^T {\text{STRAIN}}_{it}.
\end{equation}
...
We also use average STRAIN to assess statistical properties of our metric in Section \ref{sec:statprop}.
...
The coefficients $\beta$ for additional information $X_{ij}$ about pass rusher $j$ in play $i$ are estimated as fixed-effects. 
Our model for the average STRAIN by pass rusher $j$ on play $i$ is as follows.
\begin{equation}
\begin{aligned}
\overline{\text{STRAIN}}_{ij} &\sim N(R_{r[ij]} + B_{b[ij]} + Q_{q[i]} + X_{ij} \cdot \beta, \sigma^2), \text{ for } i = 1, \dots, n \text{ plays} \\
...
With regards to the fixed-effects about pass rusher $j$ in play $i$, we include the length of the play (in terms of elapsed time between ball snap and a QB event), the positions of pass rusher and nearest blocker, and the number of pass blockers assigned to the pass rusher based on the PFF data.
