...
Accordingly, the strain rate [@callister2018materials] of a material measures the change in its deformation with respect to time.
...
D_{d} &\sim N(\mu_D, \sigma^2_D), \text{ for } d = 1, \cdots, \text{ \# of teams}, \\
O_{o} &\sim N(\mu_O, \sigma^2_O), \text{ for } o = 1, \cdots, \text{ \# of teams}.
\end{aligned}
\end{equation}

We consider a normal distribution to shrink the random intercepts for each player and team toward their respective group means, which is a useful property this problem's context since we do not observe the same number of plays for each player. For the team effects, this provides us with the average defense and offense team-level effects on a pass rusher's STRAIN. Due to the nested nature of players on teams, the individual pass rusher and blocker random intercepts reflect the respective player's effect relative to their team effects. We implement the model using penalized likelihood via the `lme4` package in `R` [@bateslme4; @R2023Language].

With regards to the fixed-effects about pass rusher $j$ in play $i$, we include a variety of features that likely contribute to variation in STRAIN. First, we adjust for the position of both the pass rusher and nearest blocker to account for any positional effects. Table \ref{tab:pos} shows our positional categorization for the pass rush and pass block roles. These are encoded as indicator variables with defensive ends and tackles as the reference groups for the pass rushers and blockers respectively. We also adjust for the number of pass blockers on the play, since teams may decide to employ a more protective scheme that could lower the observed STRAIN. Finally, we also adjust for the length of the play (i.e., the elapsed time between the ball snap and a quarterback event). We expect a longer display to display lower STRAIN since this would indicate a quarterback has more time to throw the ball. NEED TO THINK ABOUT THIS MORE....
...
In order to provide a measure of uncertainty regarding about our random effects, we use a bootstrapping strategy similar to the approach in @yurko2019nflwar. Specifically, we resample team drives within games. This preserves the fact that team schedules are fixed, but allows for random variation in player usage since this is dependent on team decision making. By resampling plays within the same drive together, our sampling is generating realistic simulated data in comparison to sampling individual plays. For each resampled dataset, we fit the aforementioned multilevel model to generate a distribution of estimates for the considered player and team effects.
