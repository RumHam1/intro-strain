...
(mention avg strain can be used to correlate with pressure...)
...
While average STRAIN is a simple first step for evaluating the pass rush, the observed average STRAIN on a single play is likely due to a convolution of factors.
...
Additionally, based on previous work by PFF, the opposing QB plays an important role on observed pressure in passing plays [@eager2019pressure].
...
To this end, we fit a multilevel model to evaluate pass rushers' impact on the average STRAIN observed in a play, while accounting for their pass blocker, QB, and play-level information.
We identify the pass blocker linked with the pass rusher of interest using the data provided by PFF.
Since there can be multiple blockers linked to a rusher, for simplicity we only consider the nearest blocker positioned to the pass rusher at the start of the play.
We use random intercepts for the three player groups: (1) pass rushers as $R$, (2) nearest pass blockers as $B$, and (3) QBs as $Q$.
The coefficients $\beta$ for additional information $X_{j,i}$ about pass rusher $j$ in play $i$ are estimated as fixed-effects. 
Our model for the average STRAIN by pass rusher $j$ on play $i$ is as follows.
\begin{equation}
\begin{aligned}
\overline{\text{STRAIN}}_{j,i} &\sim N(R_{r[j,i]} + B_{b[j,i]} + Q_{q[i]} + X_{j,i} \cdot \beta, \sigma^2), \text{ for } i = 1, \dots, n \text{ plays} \\
R_{r} &\sim N(\mu_R, \sigma^2_R), \text{ for } r = 1, \cdots, \text{ \# of pass rushers}, \\
...
With regards to the fixed-effects about pass rusher $j$ in play $i$, we include the length of the play, the pass rusher position, the nearest pass blocker's position, and the number of pass blockers assigned to the pass rusher based on the PFF data.
Table \ref{tab:pos} shows our categorization of position for the pass rush and pass block roles.
...
Pass rush & Interior linebacker (middle and inside linebackers) \\ 
Pass rush & Secondary (cornerback, free safety, and strong safety) \\
\hline
Pass block & Center \\ 
Pass block & Guard \\ 
Pass block & Tackle \\ 
