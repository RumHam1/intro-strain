...
In recent years, tracking data have replaced traditional box-score statistics and play-by-play data as the state-of-the-art in sports analytics.  Numerous sports are collecting and releasing data on player and ball locations on the playing surface over the course of a game. This spatio-temporal multi-resolution source of data has provided exceptional opportunities for researchers to perform advanced studies at a more granular level to deepen our understanding of different sports.  For complete surveys on how tracking data have transformed sports analytics, see @Macdonald2020Recreating, @Baumer2023Big, and @Kovalchik2023Player.

<!-- We contribute to the growing literature of research based on player tracking data in American football.  -->

In an attempt to further develop analytics and innovate the game, the National Football League (NFL) introduced their player tracking system known as Next Gen Stats (NGS) in 2016 [@nfl2023ngs].
...
A hit is a collision between a defender and the opposing team's quarterback while the QB is in possession of the ball.  <!-- This is a sack, right? -->
...
In addition to these individual binary metrics, the sum of sacks, hits, and hurries is often defined as pressures.
...
Pro football focus (PFF) defines a metric called pass-rush productivity (PRP), which is a minor modification from the aforementioned pressures metric (see Table \ref{tab:metrics}).
In particular, PRP gives twice as much weight to a sack relative to hurries and hits, which is a small upgrade to pressures, however, the choice of weights is ad-hoc and still only copnsiders binary outcomes, similar to the shortcomings of previous metrics.
...
However, this depends on the rather arbitrary time threshold used to define a pass rush win.  In addition, once a cutoff is chosen, PRWR converts continuous data to a win-loss indicator, becoming dichotomous like most of the metrics discussed above.  
...
We propose STRAIN, a metric for measuring pass rush effectiveness, inspired by the concept of strain rate in materials science [@callister2009materials].
