...
This spatio-temporal multi-resolution source of data has provided exceptional opportunities for researchers to perform advanced studies at a more granular level to deepen our understanding of different sports.
...
The data captures real-time on-field information such as locations, speeds, and accelerations of all 22 players (and also the football).
...
Besides, once a cutoff is chosen, PRWR converts continuous data to a win-loss indicator, becoming dichotomous like most of the metrics discussed above.  
...
Additionally, we consider a multilevel model to estimate every pass rusher's contribution to the average STRAIN in a play while controlling for other positions like QB and pass blocker as well as various play-level information.
We note that although our focus in this paper is on pass rushers, our proposed approach can be extended to the evaluation of pass blockers in American football.  
...
We then introduce the mathematical motivation and definition of our measure STRAIN and our modeling approach in Section \ref{sec:methods}.
