...
Officially tracked by the NFL since 1982, the most well-known statistic for evaluating defensive linemen on pass rushing plays is the sack.
A sack is recorded when a defensive player tackles the quarterback behind the line of scrimmage before the quarterback is able to throw a pass.
...
Measures such a hits and hurries are collected by various outlets where hits counts the number of times a QB is hit while passing and hurries counts how often a QB has to throw the ball earlier than they would have liked to.  
...
More recently metrics such as Time In Pocket (TIP) and Pass Rush Win Rate (PRWR) [@Burke2018Created] have also been proposed and are substantial improvements over the less sophisticated counting statistics.
...
PRWR is based on whether a pass rusher is able to beat their block before a fixed time (ESPN chooses 2.5 seconds) from the snap, but this is highly dependent on the rather arbitrary choice of time used to define a "win".
In addition, once a cut-off is chosen, this metric becomes binary like most of the previously discussed measures.   
...
We then introduces the formal definition of our metric STRAIN and our modeling approach in Section \ref{sec:methods}.
