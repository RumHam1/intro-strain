...
$$.
This metric gives twice as much weight to a sack relative to hurries and hits, which is a slight improvement, but the choice of weights is ad-hoc an still measures binary outcomes. (https://www.pro-football-reference.com/about/advanced_stats.htm)

More recently metrics such as Time In Pocket (TIP) and Pass Rush Win Rate (PRWR) have also been proposed and are substantial improvements over the less sophisticated counting statistics.  However, these are still imperfect measures as TIP is highly context dependent (e.g. deep vs shallow passing routes).  PRWR is based on whether a pass rusher is able to beat their block before a fixed time from the snap, but this is highly dependent on the rather arbitrary choice of time used to define a "win".  

All of these measures represent different versions of "the best we can do with the available data".  Without more sophisticated data (i.e. player tracking data), it is difficult to measure effectiveness of a pass rusher.  
