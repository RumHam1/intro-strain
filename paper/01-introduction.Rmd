...
The data captures real-time on-field information such as locations, speeds, and accelerations of all 22 players (and the football).
...
There are two main types of defensive linemen in American football: (1) defensive tackles (DTs) and (2) defensive ends (DEs).
Typically, DTs and DEs are located within the interior of the line and along the edges, respectively; see Figure \ref{fig:formation} (top) for example formation with DTs and DEs.
The primary purpose of both positions is to rush the QB on passing plays, with DEs displaying superiority in observed pass rushing ability [@Eager2018nfl].
Additionally, within DTs there are nose tackles (NTs) who directly line up across from the ball at the line of scrimmage; see Figure \ref{fig:formation} (bottom) for example defensive scheme with a NT.
NFL teams often employ either one NT or two DTs on the interior with DEs along either side of the defensive line.
In addition to defensive lineman, other positions may attempt to rush the QB on blitzing plays such as outside linebackers (OLBs), interior linebackers (ILBs), and potentially members of the secondary (cornerback, free safety, strong safety) whose primary role is pass coverage.
...
```{r formation, fig.width=4.8, fig.height=3.5, fig.pos="t", fig.cap="Two common defensive alignments in football: 4-3 defense (top) and 3-4 defense (bottom). The dashed line represents the line of scrimmage separating the defense and offense. Cornerbacks (CB) and safeties (S) are primarily involved in defending against passing plays.  Outside linbackers (OLB) and inside linebackers (ILB) generally play directly behind the defensive line and are involved in defending against passing and rushing plays.  Defensive tackles (DT), defensive ends (DE), and nose tackles (NT) primarily rush the quarterback on passing plays and attempt to stop the rusher as quickly as possible on running plays."}
