...
Table \ref{tab:metrics} gives a summary of existing pass-rush metrics.
We now highlight what these quantities measure as well as their limitations.

Perhaps the most commonly-known statistics for evaluating defensive linemen on pass rush plays are sacks, hits, and hurries, which are observed at the discrete play-level.
Officially tracked by the NFL since 1982, a sack is recorded when a defensive player tackles the quarterback behind the line of scrimmage before the quarterback is able to throw a pass.
Sacks represent a simple binary measure of success for a rusher on a pass play.
However, there are many intermediate outcomes for a rusher that can also be considered successes that do not result in a sack.
Other traditional box-score statistics such hits and hurries are collected by various outlets where hits counts the number of times a QB is hit while passing and hurries counts how often a QB has to throw the ball earlier than they would have liked to.
The sum of sacks, hits, and hurries has been defined as "pressures" and is an improvement over sacks, but suffers from problems of subjectivity (e.g. was the play actually a hurry?) and are still binary outcomes (i.e. There is a continuum of plays that aren't hurries that all counted as simply "not hurries").
...
In particular, PRP gives twice as much weight to a sack relative to hurries and hits, which is a slight improvement to pressures.
However, the choice of weights is ad-hoc and still only measures binary outcomes similar to the shortcomings of previously discussed metrics.
...
More recently proposed metrics such as Time In Pocket (TIP) and Pass Rush Win Rate (PRWR) [@Burke2018Created] are substantial improvements over the less sophisticated counting statistics.
Nevertheless, these are still imperfect measures.
TIP is highly context dependent, as it can be influenced by the type of passing route (e.g. deep versus shallow).
PRWR is created using player tracking data, which is at a more granular level than previous measures.
It demonstrates whether a pass rusher is able to beat their block before a fixed time (ESPN chooses 2.5 seconds) from the snap.
This is highly dependent on the rather arbitrary choice of time used to define a "win".
In addition, once a cut-off is chosen, this metric converts continuous data to a win/loss indicator, becoming binary like most of the previously discussed measures.
...
The literature on the performance of linemen, either offensive or defensive in football is scant.
...
Later, @AlamarGoldner2011 use manually-tracked data during the 2010 season to estimate lineman performance for different team-positions instead of individual defenders (e.g. Chicago Bears' left tackle, Pittsburgh Steelers' center).
The paper uses survival analysis to model time in pocket and completion percentage for quarterbacks, before ultimately measures a lineman's contribution to the overall offense in terms of yards gained.
...
<!-- [this next part needs re-wording] -->
<!-- They then go on to use a survival analysis model to estimate time in pocket.  The average probability of success from the survival model is then used as a covariate in a model to estimate the probability of competing a pass, and they show that there is a significant relationship between these two probabilities. -->

## Contributions of STRAIN

All the aforementioned methods suffered from not having the data that was actually necessary to develop a meaningful metric for linemen in football, either offensive or defensive.
Now though, thanks to the 2023 Big Data Bowl, we have a publicly available tracking data not only for the linemen, but for every player on the field.
This allows us to meaningfully assess the performance of linemen in football.
In this paper, we focus on defensive linemen, and propose STRAIN, a metric for pass rush evaluation inspired by the concept of strain rate in materials science.
STRAIN gives a continuous measure of pressure for each of the defensive linemen over the course of an entire play.
This allows for the measurement of pass rush success even on plays that do not result in an observed outcome like a sack, hit, or hurry.
We view this as a major step forward in the accurate evaluation of defensive linemen performance.
Further, we note than while we focus exclusively on defensive linemen here, the proposed metric STRAIN is relatively easily extended to the evaluation of offensive linemen performance.  

The remainder of this paper is outlined as follows.
We first describe the player tracking data provided by 2023 Big Data Bowl in Section \ref{sec:data}.
We then introduces the mathematical motivation and definition of our metric STRAIN and our modeling approach in Section \ref{sec:methods}.
We present an real-game illustration of the proposed metric and study its performance in Section \ref{sec:results}.
We close with a discussion of future directions for STRAIN in Section \ref{sec:discussion}.
