...
Tracking data in recent years, NFL
...
meta metrics: discriminant, stability, independence @Franks2016Meta
...
## Previous Metrics for Pass Rush Evaluation
...
$$f_{d_{ij}}(t) = \sqrt{\left(x_{ijt} - x^{QB}_{it}\right)^2 + \left(y_{ijt} - y^{QB}_{it}\right)^2}$$.

* The rate at which player $j$ is moving towards the quarterback at frame $t$ during play $i$ is 

$$\displaystyle f'_{d_{ij}}(t) = \frac{df_{d_{ij}}(t)}{dt}$$.
...
To determine the most effective pass-rushers, we compute $\overline{\text{STRAIN}}$, which represents the average STRAIN across all frames played for each player.
...
The following tables show the top performing pass-rushers (with at least 100 plays) rated by $\overline{\text{STRAIN}}$ for the first 8 weeks of the 2021 season.
