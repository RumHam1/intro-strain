...
Through multilevel modeling, we observe that the pass rusher explains more variation in play-level STRAIN, followed by the quarterback and then their assigned blocker.
This is an intuitive result, consistent with previous literature on pass rushing and pressure.
However, we ignore the fact that pass rushers can be teammates cooperating in the same play leading to some type of correlation structure in their STRAIN values.
One way to address this complication could be with a adjusted plus-minus (APM) regression approach [@matano2023augmenting], with all players accounted for in modeling a total STRAIN quantity. 
...
Moreover, although we focus solely on pass rushers in this paper, STRAIN can be easily extended to the assessment of pass blockers as a unit.
This can be accomplished through the aforementioned APM regression or by simply looking at quantities such as the total STRAIN or the maximum STRAIN per frame aggregated across the entire play.
...
There is ample opportunity for working with the complete STRAIN trajectories via functional data analysis to help understand how STRAIN affects offensive production. 
